// headroom-medium.jsx — Übersicht desktop widget: the Medium (438×206)
// liquid-glass headroom card (headline + per-account session bars), fed by the
// real headroom_widget@1 projection over the loopback tunnel
// (`headroom serve`, default http://127.0.0.1:8377).
//
...
/* ---- markup builders (names escaped; values are code-built) ----
   Copied from dashboard/template.html hrMediumMarkup. */
function hrMediumMarkup(v) {
  const rows = v.accts.map((a) => {
    let toneCls, fill, value, dim = "";
    if (a.state === "current") { toneCls = a.v5.tone; fill = a.v5.fill == null ? 0 : a.v5.fill; value = a.v5.value; }
    else if (a.state === "limited") { toneCls = "red"; fill = 2; value = a.v5.value; }
    else if (a.state === "stale") { toneCls = "unknown"; fill = a.v5.fill == null ? 0 : a.v5.fill; value = a.v5.value; dim = " is-dim"; }
...
export const render = ({ output, error }) => {
  ...
  return (
    <div className="hr" data-theme={THEME}>
      ...
      <div dangerouslySetInnerHTML={{ __html: hrMediumMarkup(v) }} />
    </div>
  );
};
